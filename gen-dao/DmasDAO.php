<?PHP
	class DmasDAO {
		...
		public function getDmasByAttributeMapInRange($fkMap, $r1, $r2) {
			$dbh=$this->dbh;
			$qStr="SELECT * FROM dmas WHERE " . self::generateFilter($fkMap) . " LIMIT ? OFFSET ?";
			$q=$dbh->prepare($qStr);
		...
		public function getAllDmassInRange($r1, $r2) {
			$dbh=$this->dbh;
			$q = $dbh->prepare('SELECT * FROM dmas LIMIT ? OFFSET ?');
		...
		public function getAllDmass() {
			$dbh=$this->dbh;
			$q = $dbh->prepare('SELECT * FROM dmas');
			$q->execute();
			$returnTuples=array();
			while(($rs=$q->fetch(PDO::FETCH_OBJ))) {
				array_push($returnTuples,$rs);
		...
		public function getDmasByAttributeMap($fkMap) {
			$dbh=$this->dbh;
			$qStr="SELECT * FROM dmas WHERE " . self::generateFilter($fkMap);
			$q=$dbh->prepare($qStr);
		...
		public function insertDmas($map) {
			$dbh=$this->dbh;
			$genQuery = "INSERT INTO dmas %s VALUES %s";
			$colNames = "(";
			$colVals = "(";
			$valArr = array();
			foreach($map as $k => $v) {
		...
		public function updateDmas($updateMap, $filterMap) {
			$dbh=$this->dbh;
			$genQuery = "UPDATE dmas SET %s WHERE %s";
		...
		public function deleteDmas($deleteMap) {
			$dbh=$this->dbh;
			$genQuery = "DELETE FROM dmas WHERE " . self::generateFilter($deleteMap);
			$q=$dbh->prepare($genQuery);
