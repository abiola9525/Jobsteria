<?PHP
	...
	if(!isset($_POST["budget_upper_bound"]) || !isset($_POST["job_projected_end_date"]) || !isset($_POST["job_start_date"]) || !isset($_POST["description"]) || !isset($_POST["budget_lower_bound"]) || !isset($_POST["name"]) || !isset($_POST["currency_type"]) || !isset($_POST["remote"]) || !isset($_POST["cid"]) || !isset($_POST["jid"])) {
		die("Failed: Incomplete Request Invocation!");
	}
	
	/** TODO: Ensure that the user updating has correct credentials. 
	 * 
	 * */
	$jid = $_POST["jid"];
	$cid = $_POST["cid"];
	...
	if(!$usrProjDao->userOwnsProject($uid, $jid)) {
		die("Error: You do not have authorization to update this job.");
	}
	...
	try {
		...
		$projDao->updateProject($attrMap, array("pid" => $jid));
		...
		$projCat->updateProject_category(array("cid" => $cid), array("pid" => $jid));
