...
<?php
	...
	if($jid != null) {
		$projResults = $exProjDao->getProjectByPid($jid);
		$project = $projResults[0];
		
		$title = $project->name;
		$description = $project->description;
		$budgetUB = $project->budget_upper_bound;
		$budgetLB = $project->budget_lower_bound;
		$currencyType = $project->currency_type;
		$status = $project->status;
		$jobStart = $project->job_start_date;
		$jobEnd = $project->job_projected_end_date;
		$remote = $project->remote;
		$country = $project->project_country;
		$state = $project->project_state;
		$city = $project->project_city;
		$addr = $project->project_address;	
	}
...
<form name="post_job"  id="postJobForm" action="RegisterJob.php" method="POST">
	<div id="compSet1">
		<fieldset>
			...
			<p>Job Title: <em class="required">*</em><input type="input" name="name" id="name" value="<?php echo $title; ?>" size="80" /></p>
			<p>Description: <em class="required">*</em></p>
			<textarea rows="10" cols="80" name="description" id="description" wrap="hard"><?php echo $description; ?></textarea><br />
		</fieldset>
		<fieldset>
		...
		<select name="budget_lower_bound" id="budget_lower_bound">
		 ...
		 <fieldset>
		 	...
		 	<?php 
		 		...
		 		$exProjCatDao = new ExProjectCategoryDAO($dbh);
		 		$results = $exCatDao->getCategoriesLexicographically();
		 		
		 		$arr = $exProjCatDao->getProject_categoryByAttributeMapInRange(array("pid" => $jid),1,0);
		 		if($arr != null) {
					$projCat = $arr[0];
				}
		 		...
		 		foreach($results as $k => $v) {
					...
					$option = "<option value=\"" . $v->cid . "\"";
					
					if($projCat->cid == $v->cid) {
        ...
        <fieldset>
			...
			Job Start Date:<em class="required">*</em><input class=".ui-datepicker" type="input" name="job_start_date" id="job_start_date" value="<?php echo $jobStart; ?>" />
			Job Projected End Date:<em class="required">*</em><input class=".ui-datepicker" type="input" name="job_projected_end_date" id="job_projected_end_date" value="<?php echo $jobEnd; ?>" />
		</fieldset>
        </p>
		...
		<fieldset id="jobLocation" style="display:none;">
			...
			<p>City: <input type="text" name="project_city" id="project_city" value="<?php echo $city; ?>" /></p>
			<p>Address: <input type="text" name="project_address" id="project_address" value="<?php echo $addr; ?>" /></p>
...
function invokeRegisterJob(cb) {
	...
	$.post(base + "UpdateJobService.php",{budget_upper_bound: vbudget_upper_bound,
...
function invokeGetCities(cb) {
	...
	function doSelections() {
		...
		var budget = "<?php echo $budgetLB . '-' . $budgetUB; ?>";
		var remote = "<?php echo $remote; ?>";
		var country = "<?php echo $country; ?>";
		var state = "<?php echo $state; ?>";
		
		doSelect(document.getElementById("currency_type"), currencyType);
		doSelect(document.getElementById("budget_lower_bound"), budget);

		
		
		if(remote == "Y") {
			document.getElementById("remoteY").checked=true;
		}
		else {
			document.getElementById("remoteN").checked=true;
			document.getElementById("jobLocation").style.display="block";
			doSelect(document.getElementById("project_country"), country);
			invokeGetRegions(invkGetRegionsCb);
			doSelect(document.getElementById("project_state"), state);
		}
	}

	function doSelect(elm, v) {
